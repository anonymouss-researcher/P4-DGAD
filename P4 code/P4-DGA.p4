// Intrinsic metadata file: /home/lubuntu/bf-sde-9.2.0/install/share/p4c/p4_14include/tofino2

/* Todos
- Check in the pcap file, how many dns answers we miss in this parsing (in percentage)
- Check if we can do recirculation to parse more labels
*/
// Now parsing 4 labels (19, 19, 19, 16)
#include <core.p4>
#include <tna.p4>
...
// #define TIMEOUT 1000000 // 1 seconds
...
#define CONFIDENCE_THRESH 900 

typedef bit<48> MacAddress;
typedef bit<32> IPv4Address;
typedef bit<128> IPv6Address;
typedef bit<32> known_domain_id;
typedef bit<32> BigramVal; // When this value changes, you need to change the index of the variables "is_random[31:31] and eg_md.recirculate...flag[31:31]"

// for time
// header PTP_delay_h {
//     bit<40> ingress_tstamp;
//     bit<8> elapsed_time_offset;
//     bit<8> udp_checksum_offset;
// }
...
header recirculate_h {
    ...
    @flexible BigramVal ans;
    bit<8> i;
    @flexible bit<16> rnd_nxds;
    @flexible bit<16> all_nxds;
    @flexible BigramVal dns_reqs;
    @flexible BigramVal ip_reqs;
    @flexible bit<32> i_arrival;
    bit<7> _padding0;
    bit<1> flag;
    bit<32> src_ip;
...
struct dga_NXDomain_t {
    bit<16> nb_random_NXDomains;
    bit<16> nb_normal_NXDomains;
}

struct dga_cisc_t {
    bit<16> nb_DNS_requests;
    bit<16> nb_unique_ips;
}
struct domainid_t { 
    bit<32> domain_id;
}
...
parser SwitchIngressParser(packet_in pkt,
    ...
    // state div_loop {
    //     bit<8> res = p.recirculate.flag;
    //     transition select(res[7:7]){
    //         1: update_flag;// b_powi - a is negative, flag must be 255
    //         _: accept;
    //     }
    // }
    ...
    // state update_flag {
    //     ig_md.do_dns = 0;
    //     p.recirculate.flag = 255;
    //     // ?? for optimization, no need to parse ethernet right ?
    //     transition accept;
    // }
    ...
    state parse_dns_q1_len1 {
        ...
        transition select (p.recirculate.recirculate_bit) {
            RECIRC_CHARS: parse_dns_query2; // if recirculate header is valid and recirculate_bit == 10 --> label_len > 1
            default: set_single_char_metadata; // 1-if recirculate header is valid and recirculate_bit anything other than 10
                                      // 2- if recirculate header is not valid
        }
    }
    ...
    state parse_dns_q1_gr7 {
        // set special recirculation flag 
        pkt.extract(p.q1_part1);
        pkt.extract(p.q1_part2);
        pkt.extract(p.q1_part4);
...
parser SwitchEgressParser(
    ...
    state check_recirculate {
        ...
        // pkt.extract(recirculate_metadata);
    }
    state parse_recirculate {
...
control SwitchEgressDeparser(
    ...
    apply {
        pkt.emit(eg_md.recirculate_metadata); // i am dropping the packet before reaching here
        pkt.emit(headers);
    }
}
...
control SwitchIngress(inout Parsed_packet headers,
    ...
    bit<LABEL_LENGTH_C> full_label_1; // label 1
    bit<HASH_LENGTH_C> full_label_1_hash;  

    // bit<80> concat_hashes; // 4 labels + init label
    ...
    /* Bigram */
    BigramVal randomness_score;
    // BigramVal bigram_reg_index;
    

    action is_random_act() {
        is_random = headers.recirculate.ans - RND_DOMAIN_THRESH;
    }
   ...
   bit<16> hash_last_label_var;
    ...
    table known_domain_list {
        key = {
            hash_prelast_label: exact;
            hash_last_label: exact;
            hash_concat_hashes: exact; 
            full_label_1_hash: ternary;
        }
    ...
    action map_bigram(BigramVal freq){
        randomness_score = randomness_score + freq;
    }
    ...
    apply {
        ...
        // DNS NXDomain reply 
            ...
            if (headers.q1_part4.isValid()){
                ...
                if (headers.label2.label != 0){
                    //bigram_reg_index3 = headers.q1_part4.part[15:0];
                    static_bigrams3.apply();
                    //bigram_reg_index4 = headers.q1_part4.part[23:8];
                    static_bigrams4.apply();
                    //bigram_reg_index5 = headers.q1_part4.part[31:16];
                    static_bigrams5.apply();
                    // ?? what if it's the last char in the label
                }
            }
            ...
            if (ig_md.parsed_dns_query == 1){
                ...
                // headers.recirculate.i = NB_DIVISION_RECIRC;
                headers.recirculate.recirculate_id = RECIRCULATE_ID_C;
                recirculation(68);
...
control SwitchEgress(
    ...
    action is_random_act() {
        is_random = eg_md.recirculate_metadata.ans - RND_DOMAIN_THRESH;
    }
    ...
    // ************************************************************ DNS and IP register ************************************************************
    // ************************************************************      Start        ************************************************************
    // todo: implement unique IPs in the egress control 
    // need to check if it's IP only (udp.sport and dport != 53)
    // remove from Ingress control 
    ...
    /*
    Register<dga_cisc_t, _>(TABLE_SIZE) dga_reg2;
    RegisterAction<dga_cisc_t, _, void> (dga_reg2) update_DNS_requests = {
        void apply(inout dga_cisc_t value) {

            value.nb_DNS_requests = value.nb_DNS_requests + 1;
        }
    };
    RegisterAction<dga_cisc_t, _, void> (dga_reg2) update_unique_IP_req = {
        void apply(inout dga_cisc_t value) {
            value.nb_unique_ips = value.nb_unique_ips + 1;
        }
    };
    RegisterAction<dga_cisc_t, _, bit<16>> (dga_reg2) get_DNS_requests = {
        void apply(inout dga_cisc_t register_data, out bit<16> result) {
            result = register_data.nb_DNS_requests;
        }
    };
    RegisterAction<dga_cisc_t, _, bit<16>> (dga_reg2) get_unique_IP_req = {
        void apply(inout dga_cisc_t register_data, out bit<16> result) {
            result = register_data.nb_unique_ips;
        }
    };
    */
     // ************************************************************ DNS and IP register ************************************************************
    ...
    // ************************************************************      Start        ************************************************************
    /*
    Register<dga_NXDomain_t, _>(TABLE_SIZE) nxd_reg1;
    RegisterAction<dga_NXDomain_t, _, void> (nxd_reg1) update_NXDomain = {
        void apply(inout dga_NXDomain_t value) {
            if (eg_md.recirculate_metadata.ans < RND_DOMAIN_THRESH) {
                value.nb_random_NXDomains = value.nb_random_NXDomains + 1;
            }
            else {
                value.nb_normal_NXDomains = value.nb_normal_NXDomains + 1; 
            }
        }
    };
    RegisterAction<dga_NXDomain_t, _, bit<32>> (nxd_reg1) get_NXDomain = {// data type of register, box_num_t, return type
        void apply(inout dga_NXDomain_t register_data, out bit<32> result) {

            result[31:16] = register_data.nb_random_NXDomains;
            result[15:0] = register_data.nb_random_NXDomains;
        }
    };
    */
    ...
    // ************************************************************ Rnd_NXDomain register ************************************************************
    // ************************************************************      Start        ************************************************************
    /*
    Register<dga_NXDomain_t, _>(TABLE_SIZE) dga_reg1;
    // update randomness indicator
    RegisterAction<dga_NXDomain_t, _, void> (dga_reg1) update_NXDomain_rnd = {
        void apply(inout dga_NXDomain_t value) {
            // value.
            value.nb_random_NXDomains = value.nb_random_NXDomains + 1;
            value.src_ip = eg_md.recirculate_metadata.src_ip;
            
        }
    };
    RegisterAction<dga_NXDomain_t, _, bit<32>> (dga_reg1) get_NXDomain_rnd = {
        void apply(inout dga_NXDomain_t register_data, out bit<32> result) {
            result = register_data.nb_random_NXDomains;
        }
    };
    */
    // ************************************************************ Rnd_NXDomain register ************************************************************
    ...
    // ************************************************************ NXD last timestamp register ************************************************************
    // ************************************************************      Start        ************************************************************
    // Register<bit<32>, _>(TABLE_SIZE) nxd_last_tstamp;
    // // update time of arrival of this packet (NXD)
    // RegisterAction<bit<32>, _, void> (nxd_last_tstamp) update_nxd_last_tstamp = {
    //     void apply(inout bit<32> register_data) {
            
    //         register_data = eg_intr_md_from_prsr.global_tstamp[41:10];
            
    //     }
    // };
    // RegisterAction<bit<32>, _, bit<32>> (nxd_last_tstamp) get_nxd_last_tstamp = {
    //     void apply(inout bit<32> register_data, out bit<32> result) {
    //         if (register_data == 0) {
    //             result = 0;
    //         }
    //         else {
    //             result =  eg_intr_md_from_prsr.global_tstamp[41:10] - register_data;
    //         }
            
    //         // register_data = new time
    //         register_data = eg_intr_md_from_prsr.global_tstamp[41:10];
    //     }
    // };
    // ************************************************************ NXD last timestamp register ************************************************************
    ...
    apply {
        ...
        if (eg_md.recirculate_metadata.isValid()) {
            ...
            if (eg_md.recirculate_metadata.recirculate_bit == DONE_RECIRC) {
                ...
                // eg_md.recirculate_metadata.recirculate_bit = 19;
            }
            /*
            else if (eg_md.recirculate_metadata.recirculate_bit == 19) {
                // update reg1
                ...
                // Check reg2
                else if (eg_md.recirculate_metadata.i == 3) {
                    bit<1> is_match_2 = 0;
                    ...
                    if (is_match_2 == 1) {
                        ...
                        tstamp_reg_2_update_tstamp_action.execute(ip_domain_nxd2_hash);
                        // Drop the packet
                        eg_md.recirculate_metadata.i = 3;
                    }
                    else {
                        // Check timestamp
                        bit<1> timed_out = 0;
            ...
            */
                ...
                if (proceed_packet == 1){
                    ...
                    /*
                        is_random_act();
                        if (is_random[31:31] == 1) { 
                            update_NXDomain_rnd.execute(ip56_hash);
            ...
            /*
                ...
                // Safe to proceed with the packet
                if (eg_md.recirculate_metadata.i == 5) {
                    is_random_act();
                    if (is_random[31:31] == 1) { 
                        update_NXDomain_rnd.execute(ip56_hash);
